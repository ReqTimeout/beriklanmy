...
SERVICES = [
    ...
    ('shopee-live-viewers', 'shopee live viewers', [
        'boost shopee live', 'shopee live views', 'shopee seller service',
    ...
    ('live-stream-viewers', 'live stream boost malaysia', [
        'live selling malaysia', 'live commerce malaysia', 'tiktok shop malaysia',
        'lazada seller service', 'live stream marketing']),
]
...
def priority_score(kw, source):
    ...
    if any(w in k for w in ['price', 'pricing', 'cost', 'how much', 'packages', 'rate']): s += 8
    if any(w in k for w in ['best', 'top', 'affordable', 'hire', 'trusted']): s += 6
...
def infer_intent(kw):
    ...
    if any(w in k for w in ['how to', 'what is', 'why ', 'is it', 'tips', 'guide', 'vs ']):
    ...
    for slug, primary, extras in SERVICES:
        ...
        if slug == 'facebook-ads-management':
            ...
            deep = ['live commerce agency malaysia', 'live selling service malaysia',
                    ...
                    'lazada live viewers', 'live stream production malaysia']
        for d in deep:
...
def main():
    ...
    p.add_argument('--cap', type=int, default=5500, help='Max queue size')
    args = p.parse_args()
    ...
    stats = {
        ...
        'from_expansion': sum(1 for q in queue if q['source'] == 'expansion_my'),
